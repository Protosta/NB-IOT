...
char *cmd1="AT+QMTCFG=\"ALIAUTH\",0,\"gwvcku0pZq0\",\"region1\",\"5667062f7d1fbb2400ca2565fbece09f\"";//配置MQTT版本
char *cmd2="AT+QMTOPEN=0,\"iot-as-mqtt.cn-shanghai.aliyuncs.com\",1883";//连接MQTT,服务器IP，端口号
char *cmd3="AT+QMTCONN=0,\"region1\"";//客户名，用户名
char *cmd4="AT+QMTPUB=0,0,0,0,\"/sys/gwvcku0pZq0/region1/thing/event/property/post\"";//发布主题
...
//通过串口2发送AT指令激活NB模块，连接到自己搭建的阿里云平台
//ATI查看当前模块版本
//AT+CIMI检查SIM卡是否读卡成功
//AT+CGSN=1获取IMEI唯一序列号
//AT+CGATT?检查模块能否上网
...
{
		while(atk_nb_send_cmd(cmd4,">",100))//检查WIFI模块是否在线
		{
			delay_ms(1000);
		}
		delay_ms(1000);
